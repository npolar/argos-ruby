...
module Argos
  describe KiwiSat303Decoder do
    ...
    describe "#data" do
      ...
      context "message type 0 [10-base]" do
      ...
      context "message type 0 [16-base]" do

        before do
          @kiwisat = Argos::KiwiSat303Decoder.new
          @kiwisat.sensor_data = "104A44"
          @expected = {:message_type=>0, :battery_voltage=>3.216, :transmissions=>512, :temperature=>24.2, :schedule=>"4"}
        end

        describe "#sensor_data" do
          it { expect(@kiwisat.sensor_data).to eq( [16, 74, 68]) }
        end
